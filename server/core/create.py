...
def create_base(data: CreateDB = Body(), run=Depends(get_run), env=Depends(get_env)):
    ...
    try:
        ...
        loggerinfo.info(
            f"База {data.dbname} успешно создана пользователем {data.description}."
        )
        return Response(
            content=f"База {data.dbname} успешно создана!", media_type="text/plain"
        )
