...
@drop.post("/drop_base")
def drop_base(dbname: str = Body(), run=Depends(get_run), env=Depends(get_env)):

    try:
        params = ["/usr/lib/postgresql/16/bin/dropdb", "--no-password", f"{dbname}"]

        run(params, env=env, text=True, check=True, capture_output=True)
        loggerinfo.info(f"База {dbname} успешно удалена")
        return Response(
            content=f"База {dbname} успешно удалена", media_type="text/plain"
        )
